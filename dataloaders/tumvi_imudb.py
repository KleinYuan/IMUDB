...
class TUMVIDataset(Dataset):
    ...
    def __getitem__(self, absolute_index):
        ...
        normed_future_imu[:, 3:] = normed_future_imu[:, 3:] / 9.8
        ...
        data_return = {
            ...
            'outputs': {
                'gt_masked_seq': torch.from_numpy(seq),
                'normed_input_imu': torch.from_numpy(normed_input_imu),
                'normed_future_imu': torch.from_numpy(normed_future_imu)
            },
            'gts': {
                'position': sequence_data['p_gt_data'][future_imu_start_index: last_index],
                'velocity': sequence_data['v_gt_data'][future_imu_start_index: last_index]
            },
    ...
    def read_data(self, data_dir):
        ...
        f = os.path.join(self.caches_dir, 'dataset-room1_512_16_cache.p')
